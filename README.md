# Event Manager (Спринт 9 - Microservices)

В этом репозитории хранится история разработки проекта управления мероприятиями.

## Актуальная версия (Сданная работа 9-го спринта)

Актуальный код находится в папке `EventManager.Microservices`. Старый монолитный проект (из 8-го спринта) оставлен в корне для истории.

### Архитектура актуальной версии

Система разделена на 3 независимых микросервиса (Users, Events, Bookings) и 1 разделяемый проект с контрактами. Сервисы общаются через Apache Kafka, каждый имеет свою базу данных PostgreSQL.

### Запуск системы

**Важно!** Команду `docker compose up` необходимо запускать из папки `EventManager.Microservices`.

Инструкция по запуску и подробное описание архитектуры доступны в [README.md микросервисного решения](https://github.com/dennisrotar/EventManagerAPI/blob/sprint-9/EventManager.Microservices/README.md).
