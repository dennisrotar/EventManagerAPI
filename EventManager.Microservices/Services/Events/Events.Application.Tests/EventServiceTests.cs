...
public class EventServiceTests
{
	...
	{
		// Arrange
		var cachedTop = new List<EventResponseDto>
	{
		new EventResponseDto { Id = Guid.NewGuid(), Title = "Top Event" }
	};

		_mockCache.Setup(c => c.GetAsync<List<EventResponseDto>>("events:top10", It.IsAny<CancellationToken>()))
				  .ReturnsAsync(cachedTop);

		// Act
		var result = await _service.GetTopEvents(10);

		// Assert
		Assert.Equal(cachedTop, result);
		_mockRepo.Verify(r => r.GetTopEventsAsync(It.IsAny<int>(), It.IsAny<CancellationToken>()), Times.Never);
	}

	[Fact]
	public async Task GetTopEvents_CacheMiss_CallsRepositoryAndSetsCache()
	{
		// Arrange
		var eventEntity = Event.Create("Test Event", "Desc", DateTime.UtcNow.AddDays(1), DateTime.UtcNow.AddDays(2), 100);
		var eventList = new List<Event> { eventEntity };

		_mockCache.Setup(c => c.GetAsync<List<EventResponseDto>>("events:top10", It.IsAny<CancellationToken>()))
				  .ReturnsAsync((List<EventResponseDto>?)null);

		_mockRepo.Setup(r => r.GetTopEventsAsync(It.IsAny<int>(), It.IsAny<CancellationToken>()))
		...
		Assert.Equal(eventEntity.Id, result.First().Id);
		_mockRepo.Verify(r => r.GetTopEventsAsync(10, It.IsAny<CancellationToken>()), Times.Once);
		_mockCache.Verify(c => c.SetAsync("events:top10", It.IsAny<List<EventResponseDto>>(), It.IsAny<TimeSpan>(), It.IsAny<CancellationToken>()), Times.Once);
	}
}